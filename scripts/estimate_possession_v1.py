...
def parse_args():
    p = argparse.ArgumentParser(
        description="FootballAnalysisAI - Ball-to-pitch projection + Possession v1"
    )
    ...
    p.add_argument("--output", default=r"output\possession_v1.mp4")
    p.add_argument("--jsonl", default=r"output\possession_v1.jsonl")
    p.add_argument("--max-frames", type=int, default=-1)
    p.add_argument("--acquire-distance", type=float, default=2.4)
    p.add_argument("--release-distance", type=float, default=4.0)
...
def transform_point(H: np.ndarray, xy):
    p = H @ np.array([float(xy[0]), float(xy[1]), 1.0], dtype=np.float64)
...
def team_and_role(track: dict):
    v = track.get("team_v24") or {}
    return str(v.get("team", "UNKNOWN")), str(v.get("role", "PLAYER"))
...
def draw_pitch_panel(frame, ball_xy, players, result):
    ...
    for pl in players:
        ...
        radius = 6 if pl.track_id == result.possessor_track_id else 4
        cv2.circle(panel, p(*pl.pitch_xy), radius, color, -1, cv2.LINE_AA)

    if ball_xy is not None and inside_pitch(ball_xy):
        cv2.circle(panel, p(*ball_xy), 5, (0,255,255), -1, cv2.LINE_AA)
        cv2.circle(panel, p(*ball_xy), 8, (0,0,0), 1, cv2.LINE_AA)

    x0 = frame.shape[1] - panel_w - 8
    y0 = frame.shape[0] - panel_h - 8
    if x0 >= 0 and y0 >= 0:
        roi = frame[y0:y0+panel_h, x0:x0+panel_w]
        cv2.addWeighted(panel, 0.90, roi, 0.10, 0, roi)
...
def main():
    ...
    for p in (source, ball_path, team_path, calib_path):
        if not p.exists():
    ...
    common_frames = sorted(set(ball_rows) & set(team_rows))
    if args.max_frames >= 0:
        common_frames = common_frames[:args.max_frames]
    if not common_frames:
        raise RuntimeError("No common frames between ball and team JSONL files.")
    ...
    estimator = PossessionEstimator(
        PossessionConfig(
            ...
            confirm_frames=max(1, args.confirm_frames),
            hold_missing_ball_frames=max(0, args.hold_missing_ball),
        )
    )
    ...
    print("=" * 84)
    print("FootballAnalysisAI - Ball to Pitch + Possession v1")
    print(f"Source       : {source}")
    print(f"Ball JSONL   : {ball_path}")
    ...
    with jsonl_out.open("w", encoding="utf-8") as out:
        try:
            while frame_index <= last_needed:
                ...
                calib = (
                    calibrations.get(int(calibration_frame))
                    if calibration_frame is not None
                    else None
                )

                ball_image_xy = ball_row.get("center_xy")
                ball_pitch_xy: Optional[tuple[float, float]] = None

                if calib is not None and ball_image_xy is not None:
                    H = np.asarray(
                        calib["homography_image_to_pitch"],
                        dtype=np.float64,
                    )
                    projected = transform_point(H, ball_image_xy)
                    if inside_pitch(projected):
                        ball_pitch_xy = projected

                players = collect_players(team_row)

                result = estimator.update(
                    ball_pitch_xy,
                    players,
                    ball_detected=bool(ball_row.get("detected", False)),
                    ball_predicted=bool(ball_row.get("predicted", False)),
                )
                ...
                if ball_pitch_xy is not None:
                    ...
                    stats["ball_pitch_missing"] += 1

                annotated = frame.copy()

                if ball_image_xy is not None:
                    bx, by = map(int, map(round, ball_image_xy))
                    cv2.circle(annotated, (bx,by), 9, (0,255,255), 2, cv2.LINE_AA)
                ...
                state_color = {
                    ...
                    LOOSE: (0,215,255),
                    UNKNOWN: (160,160,160),
                }.get(result.state, (200,200,200))

                cv2.rectangle(annotated, (0,0), (width,94), (18,18,18), -1)
                cv2.putText(
                    annotated,
                    f"Possession v1: {result.state}",
                    (18,30),
                ...
                owner = (
                    ...
                    if result.possessor_track_id is not None
                    and result.distance_m is not None
                    else "NONE"
                )
                cv2.putText(
                    annotated,
                    f"Owner: {owner} | conf={result.confidence:.2f}",
                    (18,58),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.50,
                    (235,235,235),
                ...
                payload = {
                    ...
                    "ball": {
                        ...
                        "pitch_xy": (
                            [round(ball_pitch_xy[0],4), round(ball_pitch_xy[1],4)]
                            if ball_pitch_xy is not None
                            else None
                        ),
                        "detected": bool(ball_row.get("detected", False)),
                        "predicted": bool(ball_row.get("predicted", False)),
                        "confidence": float(ball_row.get("confidence", 0.0) or 0.0),
                    },
                    "possession": {
                        "state": result.state,
                        "possessor_track_id": result.possessor_track_id,
                        "possessor_team": result.possessor_team,
                        "distance_m": (
                            round(result.distance_m,4)
                            if result.distance_m is not None else None
                        ),
                        "confidence": round(result.confidence,5),
                        "nearest_track_id": result.nearest_track_id,
                        "nearest_team": result.nearest_team,
                        "nearest_distance_m": (
                            round(result.nearest_distance_m,4)
                            if result.nearest_distance_m is not None else None
                        ),
    ...
    total = sum(stats[s] for s in (TEAM_A, TEAM_B, LOOSE, UNKNOWN))

    print("=" * 84)
    print("DONE - Possession v1")
