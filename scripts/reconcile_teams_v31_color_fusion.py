...
def crop_player(frame, bbox):
    """
    Team Identity V3 jersey-focused crop.

    Instead of sending the whole player box to the deep encoder,
    focus on the central upper-body / shirt region.

    This suppresses grass, shorts, legs, skin and nearby-player
    contamination while preserving the strongest team-kit signal.
    """
    ...
    # Very small detections: keep original crop.
    if ph < 12 or pw < 6:
        return player

    # Central upper-body jersey ROI.
    jx1 = int(round(pw * 0.18))
    jx2 = int(round(pw * 0.82))
    jy1 = int(round(ph * 0.12))
    jy2 = int(round(ph * 0.60))

    jx1 = max(0, min(pw - 1, jx1))
    jx2 = max(jx1 + 1, min(pw, jx2))
    jy1 = max(0, min(ph - 1, jy1))
    jy2 = max(jy1 + 1, min(ph, jy2))

    jersey = player[jy1:jy2, jx1:jx2]
...
def jersey_color_feature(crop):
    """
    Team Identity V3.1:
    explicit kit-colour signature from the jersey ROI.

    Deep embeddings can overreact to player identity, grass/background
    and texture. This descriptor deliberately emphasizes hue,
    saturation, brightness and chroma distribution.
    """
    ...
    # Hue is meaningful mainly for sufficiently saturated pixels.
    chromatic = (s >= 45) & (v >= 25)

    if np.count_nonzero(chromatic) >= 8:
        h_values = h[chromatic]
    else:
        h_values = h.reshape(-1)
    ...
    # Lab a/b channels preserve useful red-yellow-vs-dark-red
    ...
    total = float(np.linalg.norm(feature))
...
def fuse_kit_features(deep_embedding, crop):
    """
    Deep identity/texture signal + dominant jersey colour signal.

    Colour intentionally receives the larger weight because team
    identity is fundamentally a kit-recognition problem.
    """
    deep = np.asarray(
        deep_embedding,
        dtype=np.float32,
    ...
    color = jersey_color_feature(crop)

    if color is None:
        return deep.astype(np.float32)
