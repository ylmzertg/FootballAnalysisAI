...
def parse_args():
    p = argparse.ArgumentParser(
        description="FootballAnalysisAI - Team Classification v2 real-video harness"
    )
    p.add_argument(
        "--source",
    ...
    p.add_argument(
        "--output",
        default=r"output\team_classification_v2.mp4",
    )
    p.add_argument(
        "--jsonl",
        default=r"output\team_classification_v2.jsonl",
    )
    p.add_argument(
        "--max-frames",
        type=int,
...
def main():
    ...
    print("=" * 82)
    print("FootballAnalysisAI - Team Classifier v2")
        ...
        try:
            while True:
                    ...
                    for obs in observations:
                        ...
                        if obs.role_hint == REFEREE:
                            ...
                            by_id[obs.track_id] = TeamAssignment(
                                ...
                                raw_team=UNKNOWN,
                                raw_confidence=0.0,
                                reason="bootstrap_goalkeeper_hint",
                            )

                # Without PnLCalib pitch_xy the core cannot reliably decide which
                # team owns a goalkeeper. Preserve the goalkeeper role and avoid
                # pretending a low-confidence appearance guess is authoritative.
                for obs in observations:
                    if obs.role_hint != GOALKEEPER:
                        continue
                    ...
                    if a.role != GOALKEEPER:
                        ...
                        if a.confidence < 0.70:
                            a.team = UNKNOWN
                        a.reason += ";goalkeeper_role_preserved_no_pitch"
                ...
                cv2.putText(
                    frame,
                    (
                        f"Team Classifier v2 | {ready_text} | "
