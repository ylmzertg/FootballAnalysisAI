...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "FootballAnalysisAI - Team Classifier V2.3 + PnLCalib spatial role gate"
        )
    )
    p.add_argument(
        "--source",
    ...
    p.add_argument(
        "--output",
        default=r"output\team_classification_v23_pnl.mp4",
    )
    p.add_argument(
        "--jsonl",
        default=r"output\team_classification_v23_pnl.jsonl",
    )
    p.add_argument(
        "--calibration-json",
        default=r"output\team_classification_v23_pnl_calibration.json",
    )
    p.add_argument(
        "--calibration-frames-dir",
        default=r"output\v23_pnl_frames",
    )
    p.add_argument(
        "--calibration-stride",
        type=int,
        default=5,
        help=(
            "Run PnLCalib every N frames and use the nearest successful calibration. "
    ...
    p.add_argument(
        ...
        type=int,
        default=8,
        help="Do not reuse a PnL calibration farther than this many frames.",
    )
    p.add_argument(
    ...
    p.add_argument(
        ...
        type=float,
        default=1.5,
        help="Tolerance in metres before a projected foot point is declared outside pitch.",
    )
    p.add_argument(
        "--max-frames",
        type=int,
...
def main():
    ...
    print("=" * 92)
    print("FootballAnalysisAI - Team Classifier V2.3 + PnLCalib")
    print("Harness version    : 2.3-pnl-spatial-role-gate")
            ...
            while True:
                ...
                for tr in tracks:
                    ...
                    if H is None or pitch_xy is None:
                        # No geometry for this track/frame: preserve classification,
                        # but do not invent a spatial role decision.
                        obs = make_observation(tr, None)
                        if obs is not None:
                ...
                for obs in observations:
                    ...
                    enriched["team_v23"] = assignment_to_dict(a)
    ...
    print("=" * 92)
    print("DONE - Team Classifier V2.3 + PnL SPATIAL ROLE GATE")
