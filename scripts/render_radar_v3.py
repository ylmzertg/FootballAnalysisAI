...
def parse_args():
    p = argparse.ArgumentParser(
        description="FootballAnalysisAI - temporally stabilized scene-aware radar"
    )
    p.add_argument(
        "--source",
    ...
    p.add_argument("--output", default=r"output\radar_v3.mp4")
    p.add_argument("--match-state", default=r"output\match_state_v3.jsonl")
    ...
    # Temporal player-position filter.
    p.add_argument(
        "--base-jump-tolerance-m",
        type=float,
        default=2.5,
        help="Base positional tolerance between consecutive observations.",
    )
    p.add_argument(
        "--max-speed-mps",
        type=float,
        default=18.0,
        help="Additional movement allowance per second.",
    )
    p.add_argument(
        "--reacquire-gap-frames",
        type=int,
        default=10,
        help="After this many missing frames, allow a track to reacquire anywhere.",
    )
    p.add_argument(
        "--ema-alpha",
        type=float,
        default=0.35,
        help="EMA smoothing weight for new accepted positions.",
    )
    p.add_argument(
        "--median-history",
        type=int,
        default=5,
        help="Median window before EMA smoothing.",
    )
    return p.parse_args()
...
def accepted_and_smoothed_position(
    *,
    track_id: int,
    raw_pos_m: np.ndarray,
    frame_index: int,
    fps: float,
    history_by_track: dict[int, deque],
    last_accept_by_track: dict[int, tuple[int, np.ndarray]],
    smoothed_by_track: dict[int, np.ndarray],
    base_jump_tolerance_m: float,
    max_speed_mps: float,
    reacquire_gap_frames: int,
    ema_alpha: float,
):
    last = last_accept_by_track.get(track_id)

    # First observation / long gap: allow reacquisition.
    if last is not None:
        last_frame, last_raw = last
        gap = frame_index - last_frame

        if gap <= reacquire_gap_frames:
            dt = max(gap / fps, 1.0 / fps)

            allowed_distance = (
                base_jump_tolerance_m
                + max_speed_mps * dt
            )

            distance = float(np.linalg.norm(raw_pos_m - last_raw))
            ...
            if distance > allowed_distance:
                return None, {
                    ...
                    "allowed_distance_m": round(allowed_distance, 3),
                }

    history = history_by_track[track_id]
    history.append(raw_pos_m.copy())

    median_pos = np.median(
        np.stack(list(history), axis=0),
        axis=0,
    )

    prev_smoothed = smoothed_by_track.get(track_id)

    if prev_smoothed is None:
        smoothed = median_pos
    else:
        smoothed = (
            ema_alpha * median_pos
            + (1.0 - ema_alpha) * prev_smoothed
        )

    last_accept_by_track[track_id] = (
        frame_index,
        raw_pos_m.copy(),
    )
    smoothed_by_track[track_id] = smoothed.copy()
...
def main():
    ...
    for path in (source, tracking_path, pitch_path, scenes_path):
        if not path.exists():
    ...
    history_by_track = defaultdict(
        lambda: deque(maxlen=max(1, args.median_history))
    )
    last_accept_by_track = {}
    smoothed_by_track = {}
    ...
    with match_state_path.open("w", encoding="utf-8") as out:
        while True:
            ...
            analysis_enabled = bool(scene.get("analysis_enabled", False))
            ...
            if analysis_enabled:
                ...
                if H is not None:
                    for tr in tracks:
                        ...
                        smoothed, filter_info = accepted_and_smoothed_position(
                            track_id=track_id,
                            raw_pos_m=raw_pos_m,
                            frame_index=frame_index,
                            fps=fps,
                            history_by_track=history_by_track,
                            last_accept_by_track=last_accept_by_track,
                            smoothed_by_track=smoothed_by_track,
                            base_jump_tolerance_m=args.base_jump_tolerance_m,
                            max_speed_mps=args.max_speed_mps,
                            reacquire_gap_frames=args.reacquire_gap_frames,
                        ...
                        sx_m, sy_m = map(float, smoothed)

                        if not (
                            0 <= sx_m <= PITCH_LENGTH_CM / 100.0
                            and 0 <= sy_m <= PITCH_WIDTH_CM / 100.0
                        ):
                            continue

                        pitch_tracks.append({
                            **tr,
                            "pitch_position_raw_m": [
                                round(float(raw_pos_m[0]), 3),
                                round(float(raw_pos_m[1]), 3),
                            ],
                            "pitch_position_m": [
                                round(sx_m, 3),
                                round(sy_m, 3),
                            ],
                            "position_filter": filter_info,
                        })
                ...
                overlay_radar(frame, radar, args.radar_opacity)

            else:
                stats["paused_frames"] += 1

                # A shot change invalidates camera geometry and position memory.
                last_valid_H = None
                last_metrics = None
                history_by_track.clear()
                last_accept_by_track.clear()
                smoothed_by_track.clear()
            ...
            cv2.rectangle(frame, (0, 0), (w, 76), (18, 18, 18), -1)
            ...
            cv2.putText(
                ...
                (
                    ...
                    f"players: {len(pitch_tracks)} | "
                    f"jump rejects: {stats['jump_rejected']}"
                ),
                (18, 57),
                cv2.FONT_HERSHEY_SIMPLEX,
                0.52,
            ...
            if frame_index == 1 or frame_index % 25 == 0:
                ...
                print(
                    ...
                    f"jump_rejected={stats['jump_rejected']} | "
    ...
    print(f"Jump rejects          : {stats['jump_rejected']}")
