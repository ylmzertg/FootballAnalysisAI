...
class TeamClassifierV2:
    ...
    def _correct_roles(
        ...
        V2.2 rules:
        - a detector REFEREE hint becomes trusted only after sustained track-level
          dominance (minimum frames + purity ratio) and visual novelty vs both teams;
        - once trusted, that track alone teaches the referee appearance prototype;
        - automatic REF promotion is disabled until a trusted prototype exists and
          is then used only as a conservative ID-switch recovery path;
        - goalkeeper hints still wait for calibrated PnLCalib ``pitch_xy``.
        """
        ...
        for track_id, assignment in assignments.items():
            ...
            if hint == REFEREE or state.referee_trusted:
                ...
                visually_plausible_referee = (
                    novelty >= self.config.referee_hint_min_novelty
                )
        ...
        # Pass 3: if the upstream detector has a REFEREE class, never seed a
        # referee from appearance-only outliers. We wait for a track to pass the
        # detector-consensus gate above. If the upstream detector has *no* referee
        # class at all, retain a conservative appearance-only fallback so the core
        # still works with generic person/player detectors.
