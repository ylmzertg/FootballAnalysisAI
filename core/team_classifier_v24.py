...
class TeamClassifierV2Config:
    ...
    # V2.4: goalkeeper is also a temporal role consensus. A single noisy
    # detector frame can never hard-label a defender as goalkeeper.
    goalkeeper_hint_min_frames: int = 4
    goalkeeper_hint_min_total_frames: int = 8
    goalkeeper_hint_min_ratio: float = 0.65
    goalkeeper_hint_min_confidence: float = 0.55
...
class _TrackState:
    ...
    goalkeeper_hint_frames: int = 0
    non_goalkeeper_hint_frames: int = 0
...
class TeamClassifierV2:
    ...
    def __init__(self, config: Optional[TeamClassifierV2Config] = None):
        ...
        self._seen_goalkeeper_detector_hint: bool = False
    ...
    def reset(self) -> None:
        ...
        self._seen_goalkeeper_detector_hint = False
    ...
    def _correct_roles(
        ...
        - goalkeeper hints require calibrated pitch coordinates near a goal line plus temporal detector consensus.
        """
        ...
        for track_id, assignment in assignments.items():
            ...
            if hint == GOALKEEPER:
                self._seen_goalkeeper_detector_hint = True
            if hint == GOALKEEPER and item.confidence >= self.config.goalkeeper_hint_min_confidence:
                state.goalkeeper_hint_frames += 1
            else:
                state.non_goalkeeper_hint_frames += 1
        ...
        for track_id, assignment in assignments.items():
            ...
            # V2.4 goalkeeper consensus: detector GK is evidence, never an
            # immediate hard label. This rejects tracks such as 12 GK hints in
            # 66 total frames while retaining a 20/21 goalkeeper track.
            gk_total = state.goalkeeper_hint_frames + state.non_goalkeeper_hint_frames
            gk_ratio = (
                state.goalkeeper_hint_frames / gk_total
                if gk_total > 0
                else 0.0
            )
            enough_gk_hints = (
                state.goalkeeper_hint_frames >= self.config.goalkeeper_hint_min_frames
                and gk_total >= self.config.goalkeeper_hint_min_total_frames
                and gk_ratio >= self.config.goalkeeper_hint_min_ratio
            )
            near_goal = self._near_goal_line(item.pitch_xy)

            if hint == GOALKEEPER or state.goalkeeper_trusted:
                if near_goal and (state.goalkeeper_trusted or enough_gk_hints):
                    team = self._goalkeeper_team(item, observations, assignments)
                    if team in (TEAM_A, TEAM_B):
                        state.goalkeeper_trusted = True
                        assignment.team = team
                        assignment.role = GOALKEEPER
                        assignment.confidence = max(assignment.confidence, 0.82)
                        assignment.reason += (
                            f";goalkeeper_consensus:gk={state.goalkeeper_hint_frames}/"
                            f"{gk_total},ratio={gk_ratio:.2f}"
                        )
                ...
                if hint == GOALKEEPER:
                    if item.pitch_xy is None:
                        assignment.reason += ";goalkeeper_hint_waiting_for_pitch"
                    else:
                        assignment.reason += (
                            f";goalkeeper_hint_pending:gk={state.goalkeeper_hint_frames}/"
                            f"{gk_total},ratio={gk_ratio:.2f},near_goal={int(near_goal)}"
                        )
                    handled.add(track_id)
                    continue

            # V2.4 deliberately disables appearance-only goalkeeper promotion.
            # Geometry + a different shirt colour is not enough because defenders
            # can occupy the six-yard/penalty area. A track must earn temporal
            # goalkeeper detector consensus first.

            if state.goalkeeper_trusted and near_goal:
                team = self._goalkeeper_team(item, observations, assignments)
                if team in (TEAM_A, TEAM_B):
                    assignment.team = team
                    assignment.role = GOALKEEPER
                    assignment.confidence = max(assignment.confidence, 0.78)
                    assignment.reason += ";goalkeeper_trusted_recovery"
                    state.goalkeeper_votes += 1
                    state.referee_votes = max(0, state.referee_votes - 1)
                    handled.add(track_id)
